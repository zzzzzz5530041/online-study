package com.online.edu.web.controller.main;
...
public class LoginController extends BaseController {
    ...
    public String outLogin(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            //删除所有的权限缓存
            redisUtils.remove(CacheConstans.SYS_ALL_USER_FUNCTION_PREFIX + userId);
            //删除登录用户的缓存信息
            redisUtils.remove(CacheConstans.LOGIN_MEMCACHE_PREFIX + userId);
            //删除登录用户权限缓存
            redisUtils.remove(CacheConstans.USER_FUNCTION_PREFIX + userId);
            //删除页面用户Cookie
    ...
    public ModelAndView login(HttpServletRequest request, HttpServletResponse response, @ModelAttribute("sysUser") SysUser sysUser) {
        ...
        try {
            ...
            //缓存用登录信息
            redisUtils.save(CacheConstans.LOGIN_MEMCACHE_PREFIX + su.getUserId(), su);
