...
import com.online.edu.common.util.PDFUtils;
...
public class ImageUploadController extends BaseController {
            ...
            if (imgFile == null) {
                ...
                if (imgFile.getSize() > maxSize) {
                    ...
                    if (fileType.contains(ext) && !"jsp".equals(ext)) {
                        ...
                        String smallUrl = this.zoomImage(request, filePath, Integer.valueOf(width), Integer.valueOf(height));
    ...
    public String saveface(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            FileUploadUtils.changeSize(this.getProjectRootDirPath(request) + photoPath, this.getProjectRootDirPath(request) + photoPath, imageWidth, imageHeight);
            ...
            FileUploadUtils.cut(this.getProjectRootDirPath(request) + photoPath, this.getProjectRootDirPath(request) + newPath, cutLeft, cutTop, dropWidth, dropHeight);
            File file = new File(this.getProjectRootDirPath(request) + photoPath);
            if (file.exists()) {
                file.delete();
            }
    ...
    public String pdfUploadAndImg(HttpServletRequest request, HttpServletResponse response, @RequestParam(value = "param",required = false) String param) {
        ...
        try {
            ...
            if ("jsp".equals(ext)) {
                ...
                imgFile.transferTo(file);
                Map<String, String> pdfToPngmap = PDFUtils.pdfToPng(this.getProjectRootDirPath(request), this.getProjectRootDirPath(request) + filePath, width, height);
    ...
    public String uploadfile(HttpServletResponse response, HttpServletRequest request, @RequestParam(value = "param",required = false) String param) {
        try {
            ...
            logger.error("uploadfile error", var9);
            return null;
        }
    }
